    ...
    tools: [
      {
        ...
        button: true,
        onClick: () => {
          let editViewbox = controls.find(controls => controls.name == "LockView").tools.find(tools => tools.name == "EditViewbox");
          editViewbox.active = false;
          setViewDialog(controls);
...
function handleMouseDown(e) {
  ...
  for (let i = 0; i < VIEWBOX.viewbox.length; i++) {
    ...
    if (e.data.button == 0 && Math.abs(position.x - moveLocation.x) <= icons_size && Math.abs(position.y - moveLocation.y) <= icons_size) mouseMode = 'move';
    //if mouse over scale button
    else if (e.data.button == 0 && Math.abs(position.x - scaleLocation.x) <= icons_size && Math.abs(position.y - scaleLocation.y) <= icons_size) mouseMode = 'scale';
    else continue;
...
function setViewDialog(controls) {
  ...
  /*
    * 0: Reset Inital View
    * 5: Move Grid Space (keep position)
  */
  let options = `
